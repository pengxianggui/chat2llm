...
def sso_handler(client: Union[str, None] = Query(default="0", description="客户端id，必须是系统支持的，用以声明h5环境。默认是0表示浏览器环境"),
    ...
    if client is None:
        return RedirectResponse(url=f"{H5_ADDRESS}/401?message=未知客户端,无法单点登录,请重新进入。")

    if client == "0":  # h5演示页面
        return redirect_h5_demo(secret, request)
    if client == "1":  # 企微页面
...
def redirect_h5_demo(secret: str, request: Request):
    if secret != H5_DEMO_SECRET:
        return RedirectResponse(url=f"{H5_ADDRESS}/401?message=单点失败, 请提供正确口令")
