...
from configs import logger, log_verbose, LLM_MODELS, HTTPX_DEFAULT_TIMEOUT, ENABLE_LLM_MODEL
...
def stop_llm_model(
    model_name: str = Body(..., description="要停止的LLM模型名称", examples=[ENABLE_LLM_MODEL]),
    controller_address: str = Body(None, description="Fastchat controller服务器地址", examples=[fschat_controller_address()])
) -> BaseResponse:
    '''
...
def change_llm_model(
    model_name: str = Body(..., description="当前运行模型", examples=[ENABLE_LLM_MODEL]),
    new_model_name: str = Body(..., description="要切换的新模型", examples=[ENABLE_LLM_MODEL]),
    controller_address: str = Body(None, description="Fastchat controller服务器地址", examples=[fschat_controller_address()])
):
    '''
